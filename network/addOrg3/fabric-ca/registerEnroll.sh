...
function createOrg3 {
  ...
  set -x
  fabric-ca-client register --caname ca-org3 --id.name patient1 --id.secret 123 --id.type client --tls.certfiles "${PWD}/fabric-ca/org3/tls-cert.pem"
  { set +x; } 2>/dev/null
  ...
  set -x
	fabric-ca-client enroll -u https://patient1:123@localhost:11054 --caname ca-org3 -M "${PWD}/../organizations/peerOrganizations/org3.example.com/users/patient1@org3.example.com/msp" --tls.certfiles "${PWD}/fabric-ca/org3/tls-cert.pem"
  { set +x; } 2>/dev/null

  cp "${PWD}/../organizations/peerOrganizations/org3.example.com/msp/config.yaml" "${PWD}/../organizations/peerOrganizations/org3.example.com/users/patient1@org3.example.com/msp/config.yaml"
